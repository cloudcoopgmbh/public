...
#    Current Version	: 1.20 - Updated by Andres-Miguel Sichel for Server 2025. Added Check for Server 2025, .net 4.8.1, update Windows Features                 #										
...
#                                                                                                                                                                  #
# DATE RELEASED         : 11/20/2018 (07/19/2023 - Last Update)  	 																                               #
#																																								   #
...
#    Edited by          : Andres-Miguel Sichel / asichel.de (Blog) / cloudcoop.de (company )   													  	               #
...
#																																								   #
# EXECUTION																																						   #
#  .\Set-Exchange2019Prerequisites-1.20.ps1																														   #
#																																								   #
####################################################################################################################################################################
...
#####################################################################################
#         Global Functions - Windows Server 2019 / 2022 / 2025                      #
...
Function AddWindowsDefenderExclusions {
    ...
    If ($PSSnapinLoad) {
        ...
        If ($ServerRole -eq 'Mailbox') {
            ...
            Add-MpPreference -ExclusionProcess "$env:SystemDrive\inetpub\temp\IIS Temporary Compressed Files"
            ...
            Add-MpPreference -ExclusionProcess "$($Exinstall)Bin\MSExchangeMailboxReplication.exe","$($Exinstall)Bin\MSExchangeRepl.exe","$($Exinstall)Bin\MSExchangeSubmission.exe","$($Exinstall)Bin\MSExchangeTransport.exe","$($Exinstall)Bin\MSExchangeTransportLogSearch.exe","$($Exinstall)Bin\MSExchangeThrottling.exe","$($Exinstall)Bin\Search\Ceres\Runtime\1.0\Noderunner.exe","$($Exinstall)Bin\OleConverter.exe","$($Exinstall)Bin\Search\Ceres\ParserServer\ParserServer.exe","$($Exinstall)FIP-FS\Bin\ScanEngineTest.exe"
            ...
            Add-MpPreference -ExclusionProcess "$($Exinstall)FIP-FS\Bin\ScanningProcess.exe","$($Exinstall)FIP-FS\Bin\UpdateService.exe"
...
Function Check-DotNetVersion {
    ...
    If ($NETval.Release -gt "533320") {
        If ($OS -like '*2025*') {
            If ($NETval.Release -gt "533320") {
                Write-Host "Greater than .NET 4.8.1 is installed - " -NoNewLine 
    ...
    If (($OS -like '*2025*') -and ($NETval.Release -eq "533320")) {
        Write-Host ".NET 4.8.1 is installed. Suitable for Exchange 2019 CU15 or SE - " -NoNewLine 
...
Function CheckCoreMailbox {
    ...
    Clear-Host
    Write-Host '------------------------------------------------------------------------------------------' -ForegroundColor White
    Write-Host ' Checking all requirements for Exchange 2019 Mailbox Role on Windows 2019 /2022 /2025 (Core OS)' -ForegroundColor Magenta
    Write-Host '------------------------------------------------------------------------------------------' -ForegroundColor White
    Write-Host ' '
    Write-Host ' '
...
Function CheckEdgeTransport {
    Clear-Host
    Write-Host '--------------------------------------------------------------------------------------' -ForegroundColor White
    Write-Host ' Checking all requirements for Exchange 2019 Edge Transport Role on Windows 2019/2022 / 2025' -ForegroundColor Magenta
    Write-Host '--------------------------------------------------------------------------------------' -ForegroundColor White
    Write-Host ' '
    Write-Host ' '
...
Function CheckFullMailbox {
    Clear-Host
    Write-Host '-----------------------------------------------------------------------------------------' -ForegroundColor White
    Write-Host ' Checking all requirements for Exchange 2019 Mailbox Role on Windows 2019/2022/2025 (Full OS)' -ForegroundColor Magenta
    Write-Host '-----------------------------------------------------------------------------------------' -ForegroundColor White
    Write-Host ' '
    Write-Host ' '
    ...
    # .NET Check - 4.8 is default, greater and lower than that not supported as of 2019/06/13
    Check-DotNetVersion
...
# Function .NET 4.8.1
Function Install-NET481 {

    # VerIfy .NET 4.8.1 is not already installed
    Check-DotNetVersion
    $DotNetVersion = ($global:NetVersion).release
    If ($DotNetVersion -lt 533320) {

        Write-Host "  ***  .NET 4.8.1 is not installed.  Downloading now!!  ***" -ForegroundColor Yellow
        # Download .NET 4.8.1 installer
        
        FileDownload "https://download.microsoft.com/download/4/b/2/cd00d4ed-ebdd-49ee-8a33-eabc3d1030e3/NDP481-x86-x64-AllOS-ENU.exe" #.Net 4.8.1
        
	    Set-Location $DownloadFolder
        Write-Host " "
	    Write-Host "File: NDP481-x86-x64-AllOS-ENU.exe installing..." -NoNewLine

        # New Code (Waits for completion)
        .\NDP481-x86-x64-AllOS-ENU.exe /quiet /norestart | Out-Null
        
        Write-Host "`n.NET 4.8.1 is now installed" -ForegroundColor Green
        Write-Host " "
        $Reboot = $true
...
} # End of Function .NET 4.8.1 Install
...
# Run code for Windows Server 2025 Full OS
Function Code2025Full {

    $Menu = {
        Write-Host "	*****************************************************" -ForegroundColor Cyan
        Write-Host "	 Exchange Server 2025 (Full OS) Prerequisites Script" -ForegroundColor Cyan
        Write-Host "	*****************************************************" -ForegroundColor Cyan
        Write-Host " "
        Write-Host "	*** .NET 4.8.1 ***" -ForegroundColor Yellow
        Write-Host " "
        ...
        Write-Host "	23) Install - One-Off - .NET 4.8.1 - CU14+" -ForegroundColor White
        ...
        Switch ($Choice)    {
            1 {# Prep Mailbox Role - Full OS
                If ($Core) {
                    ...
                    Write-host "Windows 2019/2022/2025 Server CORE " -Foregroundcolor red -NoNewline
                    ...
                    Clear-Host
                    Write-Host "--------------------------------------------------------------------------------------" -ForegroundColor White
                    Write-Host " Installing prerequisites for Exchange 2019 Cu14+ Mailbox Role on Windows Server 2025!" -ForegroundColor Magenta
                    Write-Host "--------------------------------------------------------------------------------------" -ForegroundColor White
                    Write-host ''
            ...
            2 { # Pre Edge Transport Role - Full OS
                    Clear-Host
                    Write-Host "---------------------------------------------------------------------------------------------" -ForegroundColor White
                    Write-Host " Installing prerequisites for Exchange 2019 Edge Transport Role on Windows Server 2022!" -ForegroundColor Magenta
                    Write-Host "---------------------------------------------------------------------------------------------" -ForegroundColor White
                    Write-host ''
            ...
            10 {
                ...
                If ($Core) {
                    ...
                    Write-host "for Windows 2019/2022/2025 Server Core.  Cannot continue." -Foregroundcolor White
                    Write-host "Please choose another option." -Foregroundcolor Yellow
                    Write-host " "
                    Write-host " "
                    Start-Sleep 4
...
} # End of 2025 Full OS Install Function
...
If (($Ver -gt '10.0.17763') -and ($Ver -lt '10.0.26100')) {
    $OSCheck = $True
...
If ($OSCheck -ne $True) {
    write-host " "
    write-host "The server is not running Windows Server 2019/2022/2025.  Exiting the script."  -ForegroundColor Red
    write-host " "
    Exit
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC